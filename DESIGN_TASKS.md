...
- [x] D3.1: With the foundation from Phase 1–2 in place, enumerate
...
- [x] D4.3: `src/app/layout.tsx` — `metadata.title`/`description` are
      still the create-next-app placeholder ("Create Next App" /
      "Generated by create next app"). Update to real values.
- [x] D4.4: Remove the five unused scaffold SVGs in `public/`
      (`file.svg`, `globe.svg`, `next.svg`, `vercel.svg`,
      `window.svg`) — confirmed unreferenced anywhere in `src/`.
