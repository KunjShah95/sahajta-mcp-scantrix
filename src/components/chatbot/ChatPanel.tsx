...
type View = "chat" | "history" | "view-record";
...
export function ChatPanel({ companyName, onClose }: { companyName?: string; onClose: () => void }) {
  ...
  const userEmail: string | undefined = useAppSelector((state) => state.auth.user?.data?.user?.email);
  ...
  const [conversationId, setConversationId] = useState(() => newMessageId());
  const [historyRecords, setHistoryRecords] = useState<ConversationRecord[]>([]);
  const [viewingRecord, setViewingRecord] = useState<ConversationRecord | null>(null);
  ...
  const refreshHistory = useCallback(() => {
    if (!userEmail || !qbConnectionId) return;
    setHistoryRecords(loadConversations(userEmail, qbConnectionId));
  }, [userEmail, qbConnectionId]);

  // Auto-save current conversation to localStorage whenever messages change.
  useEffect(() => {
    if (!userEmail || !qbConnectionId || messages.length === 0) return;
    const record: ConversationRecord = {
      id: conversationId,
      userEmail,
      qbConnectionId,
      startedAt: parseInt(conversationId.split("-")[1] ?? "0", 10),
      updatedAt: Date.now(),
      messages,
      preview: buildPreview(messages),
    };
    saveConversation(record);
  }, [messages, conversationId, userEmail, qbConnectionId]);

  const startNewChat = () => {
    dispatch(clearChat());
    setConversationId(newMessageId());
    setView("chat");
  };
  ...
  const handleSelectRecord = (record: ConversationRecord) => {
  ...
  const handleContinueRecord = (record: ConversationRecord) => {
    dispatch(restoreConversation(record.messages));
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        if (res.status === 401) sessionEmitter.emit(SESSION_EXPIRED);
  ...
  const renderHeader = () => {
    if (view === "history") {
      return (
        <div className="flex h-16 shrink-0 items-center gap-[var(--space-sm)] border-b border-border px-[var(--space-lg)]">
          <button
            type="button"
            onClick={() => setView("chat")}
            aria-label="Back to chat"
            className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full text-text-secondary hover:bg-background-alt"
          >
            <ArrowLeft size={18} strokeWidth={2} />
          </button>
          <h2 className="flex-1 text-h3 font-bold text-trust-navy">Chat History</h2>
          <button
            type="button"
            onClick={onClose}
      ...
      return (
        <div className="flex h-16 shrink-0 items-center gap-[var(--space-sm)] border-b border-border px-[var(--space-lg)]">
          ...
          <div className="min-w-0 flex-1">
            <h2 className="truncate text-h3 font-bold text-trust-navy">Past Conversation</h2>
            {viewingRecord && (
              <p className="text-caption text-text-secondary">
                {new Date(viewingRecord.startedAt).toLocaleDateString()}
              </p>
            )}
          </div>
          <button
        ...
        <div className="flex shrink-0 items-center gap-1">
          {messages.length > 0 && (
            <button
              ...
              aria-label="New chat"
              title="New chat"
              className="flex h-10 w-10 items-center justify-center rounded-full text-text-secondary hover:bg-background-alt"
            >
  ...
  return (
    ...
    >
      ...
      {view === "view-record" && viewingRecord && (
        <>
          <div className="flex-1 space-y-[var(--space-md)] overflow-y-auto px-[var(--space-lg)] py-[var(--space-md)]">
            {viewingRecord.messages.map((message) => (
              <ChatMessage key={message.id} message={message} />
            ))}
          </div>
          <div className="shrink-0 border-t border-border p-[var(--space-md)]">
            <Button type="button" onClick={() => handleContinueRecord(viewingRecord)} className="w-full">
