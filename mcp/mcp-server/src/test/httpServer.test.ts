...
const startServer = (config: Config): Promise<{ baseUrl: string; close: () => Promise<void> }> =>
  new Promise((resolve) => {
...
test("POST /mcp handles initialize and tools/list", async () => {
  ...
  try {
    ...
    const toolsBody: any = await toolsRes.json();
    const names = (toolsBody.result?.tools ?? []).map((t: any) => t.name);
