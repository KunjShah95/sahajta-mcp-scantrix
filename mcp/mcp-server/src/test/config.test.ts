...
const clearEnv = () => {
  for (const k of SAVETRIX_ENV_KEYS) delete process.env[k];
};
...
test("loadConfig CLI flags override env (--http and --port)", () => {
  clearEnv();
  process.env.SAVETRIX_PORT = "9100";
  const c: Config = loadConfig(["--http", "--port", "9999"]);
...
test("loadConfig reads a config file at --config and env overrides file", async () => {
  ...
  await writeFile(
    file,
    JSON.stringify({ email: "file@test.test", qbConnectionId: "qb-1" }),
  );
  process.env.SAVETRIX_EMAIL = "env@test.test";
  const c: Config = loadConfig(["--config", file]);
