 ...
 */
const withHost = (baseUrl, path, host, init = {}) => new Promise((resolve, reject) => {
...
test("an allowlisted alias host advertises itself as issuer and resource", async () => {
    const { baseUrl, close } = await startServer(makeConfig({ allowedHosts: ["old-alias.vercel.app"] }));
    try {
        const meta = (await withHost(baseUrl, "/.well-known/oauth-protected-resource/mcp", "old-alias.vercel.app")).json();
        ...
        const as = (await withHost(baseUrl, "/.well-known/oauth-authorization-server", "old-alias.vercel.app")).json();
        ...
        assert.match(String(res.header("www-authenticate")), /resource_metadata="https:\/\/old-alias\.vercel\.app\/\.well-known\/oauth-protected-resource\/mcp"/);
    }
    finally {
        await close();
    }
...
test("a host that is not allowlisted cannot redirect discovery at itself", async () => {
    ...
    try {
        ...
        for (const forged of ["evil.example", "connector.test.evil.example"]) {
            const meta = (await withHost(baseUrl, "/.well-known/oauth-protected-resource/mcp", forged)).json();
...
test("the canonical host keeps advertising the canonical identity", async () => {
    const { baseUrl, close } = await startServer(makeConfig({ allowedHosts: ["old-alias.vercel.app"] }));
    try {
        const meta = (await withHost(baseUrl, "/.well-known/oauth-protected-resource/mcp", "connector.test")).json();
...
test("an expired or malformed access token gets 401 + a challenge, never 500", async () => {
    ...
    try {
        ...
        const expired = await encryptToken(TOKEN_SECRET, "access", { session: { client_id: "c1", st_at: "a", st_rt: "r", userId: "u1" } }, -60);
...
test("GET and DELETE /mcp are 405, not an SSE stream nothing can write to", async () => {
    const { baseUrl, close } = await startServer(makeConfig({}));
    try {
        const token = await encryptToken(TOKEN_SECRET, "access", { session: { client_id: "c1", st_at: "a", st_rt: "r", userId: "u1" } }, 3600);
