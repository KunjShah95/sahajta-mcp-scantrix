 ...
 */
export class SavetrixOAuthProvider {
    config;
    constructor(config) {
        this.config = config;
        if (!config.tokenSecret || config.tokenSecret.length < 32) {
            throw new Error("SAVETRIX_TOKEN_SECRET must be set to a random string of at least 32 characters for the remote connector.");
        }
    }
    get secret() {
    ...
    get clientsStore() {
        return {
            registerClient: async (client) => {
                const clientId = await encryptToken(this.secret, "client", { client }, CLIENT_TTL);
                return {
                    ...client,
                    client_id: clientId,
            ...
            getClient: async (clientId) => {
                try {
                    const { client } = await decryptToken(this.secret, "client", clientId);
    ...
    async readLoginRequest(token) {
        const { req } = await decryptToken(this.secret, "login_req", token);
        return req;
    }
    ...
    async challengeForAuthorizationCode(_client, authorizationCode) {
        const { code } = await decryptToken(this.secret, "code", authorizationCode);
    ...
    async exchangeRefreshToken(client, refreshToken) {
        const { session } = await decryptToken(this.secret, "refresh", refreshToken);
    ...
    async verifyAccessToken(token) {
        ...
        let decoded;
        try {
            decoded = await decryptToken(this.secret, "access", token);
        }
        catch (error) {
            throw new InvalidTokenError(error instanceof Error && /exp/i.test(error.message)
    ...
    async mintTokens(session) {
        const access_token = await encryptToken(this.secret, "access", { session }, ACCESS_TTL);
        const refresh_token = await encryptToken(this.secret, "refresh", { session }, REFRESH_TTL);
        return {
            access_token,
            token_type: "Bearer",
